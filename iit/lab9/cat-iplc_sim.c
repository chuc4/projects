...
void iplc_sim_push_pipeline_stage()
{
  ...
  if( pipeline[WRITEBACK].instruction_address )
    ...
    if( pipeline[DECODE] == BRANCH)
      {
        unsigned int check_address = 0;
        check_address = pipeline[FETCH].instruction_address - pipeline[DECODE].instruction_address;
        branch_count++;
        if(check_address == 4) //check if branch taken
          {
      ...
      if(pipeline[MEM].itype == SW)
        {
          if(iplc_sim_trap_address(pipeline[MEM].stage.store_word.data_address) == 1)
            {
              //check if data hit
              pipeline_cycles++;
            }
